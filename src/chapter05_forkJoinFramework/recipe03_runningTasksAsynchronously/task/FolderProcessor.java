package chapter05_forkJoinFramework.recipe03_runningTasksAsynchronously.task;
...
public class FolderProcessor extends CountedCompleter<List<String>> {
		...
		if (content != null) {
			for (int i = 0; i < content.length; i++) {
				if (content[i].isDirectory()) {
					...
					// fork() method sends the task to the pool that will execute it 
					// if it has a free worker thread, or it can create a new one.
					// The method returns immediately, so the current task can continue processing
					task.fork();
					addToPendingCount(1);
					tasks.add(task);
				} else {
	...
	// This method will be executed when all the child tasks
	// (all the tasks that have been forked from the current task) have
	// finished their execution, so in this method it is safe to fetch results
	@Override
