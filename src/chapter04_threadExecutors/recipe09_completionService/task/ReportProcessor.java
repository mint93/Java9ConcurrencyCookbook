package chapter04_threadExecutors.recipe09_completionService.task;
...
public class ReportProcessor implements Runnable {
	...
	public void run() {
		while (!end){
			try {
				// Alternative to poll() is take() - this method, without arguments,
				// checks whether there are any Future objects in the queue.
				// If it is empty, it blocks the thread until the queue has an element.
				// If the queue has elements, it returns and deletes it
